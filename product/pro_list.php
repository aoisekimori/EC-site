...
<body>
  ...
  <h2 class="title staff-title">商品一覧</h2>
  <?php
     try {
       ...
       $sql = 'SELECT code,name,price FROM cafe_product WHERE 1';
   ...
   <?php while(true): ?>
     ...
     <?php print '<form method="post" action="pro_branch.php">' ?>
       <ul>
        <?php print '<input type="radio" name="procode" value="'.$rec['code'].'">'; ?>
         <li style="display:inline"><?php echo $rec['name']; ?>___<?php echo $rec['price']; ?>円</li>
       </ul>
  <?php endwhile; ?>
